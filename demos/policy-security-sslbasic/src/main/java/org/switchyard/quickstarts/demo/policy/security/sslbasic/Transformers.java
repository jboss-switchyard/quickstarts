...
public class Transformers {
    ...
    @Transformer(from = "{urn:switchyard-quickstart-demo:policy-security-sslbasic:0.1.0}doWork")
    ...
    @Transformer(to = "{urn:switchyard-quickstart-demo:policy-security-sslbasic:0.1.0}doWorkResponse")
    public Element transformToElement(WorkAck workAck) {
        StringBuilder ackXml = new StringBuilder()
            .append("<policy-security-sslbasic:doWorkResponse xmlns:policy-security-sslbasic=\"urn:switchyard-quickstart-demo:policy-security-sslbasic:0.1.0\">")
            .append(    "<workAck>")
            .append(        "<command>" + workAck.getCommand() + "</command>")
            .append(        "<received>" + workAck.isReceived() + "</received>")
            .append(    "</workAck>")
            .append("</policy-security-sslbasic:doWorkResponse>");
