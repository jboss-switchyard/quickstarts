 ...
 */
public class CamelCxfRsBindingClient {
    ...
    public static void main(String[] args) throws Exception {
        String command =  null;
        if (args.length == 0) {
            System.out.println("Usage: CamelCxfRsBindingClient new|get|add|del [orderId] [itemId]");
            ...
            if (command.equals("new")) {
                String response = http.sendString(BASE_URL + "/order/inventory", "", HTTPMixIn.HTTP_GET);
                if (response.equals("false")) {
                    http.sendString(BASE_URL + "/order/inventory/create", "", HTTPMixIn.HTTP_OPTIONS);
                }
