...
public class WarehouseProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        SOAPHeaderElement replyTo = (SOAPHeaderElement) exchange.getIn()
            .getHeaders()
            .get("{http://www.w3.org/2005/08/addressing}ReplyTo");
        SOAPHeaderElement faultTo = (SOAPHeaderElement) exchange.getIn()
            .getHeaders()
            .get("{http://www.w3.org/2005/08/addressing}FaultTo");
        SOAPHeaderElement relatesTo = (SOAPHeaderElement) exchange.getIn()
            .getHeaders()
            .get("{http://www.w3.org/2005/08/addressing}RelatesTo");
        ...
        if (order.getItem().equals("Guardian Angel")) {
            ...
            String toStr = "<wsa:To xmlns:wsa=\"http://www.w3.org/2005/08/addressing\">"
                + ((Node) replyTo.getChildElements().next()).getFirstChild().getNodeValue() + "</wsa:To>";
