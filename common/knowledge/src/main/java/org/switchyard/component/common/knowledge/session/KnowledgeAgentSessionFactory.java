...
import org.switchyard.component.common.knowledge.util.Agents;
...
/**
 * An Agent-based KnowledgeSessionFactory.
 *
 * @author David Ward &lt;<a href="mailto:dward@jboss.org">dward@jboss.org</a>&gt; &copy; 2012 Red Hat Inc.
 */
class KnowledgeAgentSessionFactory extends KnowledgeSessionFactory {

    private final KnowledgeAgent _agent;
    private final KieSessionConfiguration _sessionConfiguration;

    KnowledgeAgentSessionFactory(KnowledgeComponentImplementationModel model, ClassLoader loader, ServiceDomain domain, Properties propertyOverrides) {
        super(model, loader, domain, propertyOverrides);
        _agent = Agents.newAgent(getModel(), getPropertyOverrides(), getLoader());
        addDisposals(Disposals.newDisposal(_agent));
    ...
    public KnowledgeSession newStatelessSession() {
        StatelessKieSession stateless = _agent.newStatelessKnowledgeSession(_sessionConfiguration);
    ...
    public KnowledgeSession newStatefulSession(Map<String, Object> environmentOverrides) {
        ...
        KieSession stateful = _agent.getKnowledgeBase().newKieSession(_sessionConfiguration, env);
    ...
    public KnowledgeSession getPersistentSession(Map<String, Object> environmentOverrides, Integer sessionId) {
        ...
        if (sessionId != null) {
            stateful = kieStoreServices.loadKieSession(sessionId, _agent.getKnowledgeBase(), _sessionConfiguration, env);
        }
        if (stateful == null) {
            stateful = kieStoreServices.newKieSession(_agent.getKnowledgeBase(), _sessionConfiguration, env);
        }
        KnowledgeDisposal loggersDisposal = Loggers.registerLoggersForDisposal(getModel(), getLoader(), stateful);
        Listeners.registerListeners(getModel(), getLoader(), stateful);
        // channels are only meaningful for stateful sessions
