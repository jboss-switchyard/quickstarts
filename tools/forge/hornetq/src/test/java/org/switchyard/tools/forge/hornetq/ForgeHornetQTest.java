...
/**
 * Test for {@link HornetQFacet}.
 */
public class ForgeHornetQTest extends GenericTestForge {
    ...
    private static final String HORNETQ_SUCCESS_MSG = "***SUCCESS*** Installed [switchyard.hornetq] successfully";
    
    private static final String HORNETQ_BINDING_MSG = "Added binding.hornetq to service " + BEAN_SERVICE;
    
    private static final String HORNETQ_REFERENCE_MSG = "Added binding.hornetq to reference " + BEAN_SERVICE_REFERENCEABLE;
    ...
    public static JavaArchive getDeployment() {
        ...
        archive.addPackages(true, HornetQFacet.class.getPackage());
    ...
    private void testBindService() throws Exception {
        resetOutputStream();
        getShell().execute("project install-facet switchyard.hornetq");
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(HORNETQ_SUCCESS_MSG));
        resetOutputStream();
        ...
        getShell().execute("hornetq-binding bind-service --queueName jms.queue.GreetingServiceQueue --serviceName " + BEAN_SERVICE);
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(HORNETQ_BINDING_MSG));
    }
    ...
    private void testBindReference() throws Exception {
        ...
        getShell().execute("hornetq-binding bind-reference --queueName jms.queue.GreetingServiceQueue --referenceName " + BEAN_SERVICE_REFERENCEABLE);
        getShell().execute("switchyard show-config");
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(HORNETQ_REFERENCE_MSG));
    }
