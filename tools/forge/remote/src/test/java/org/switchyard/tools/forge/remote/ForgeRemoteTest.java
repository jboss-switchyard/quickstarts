 ...
 */
public class ForgeRemoteTest extends GenericTestForge {
    ...
    private static final String REMOTE_SUCCESS_MSG = "***SUCCESS*** Installed [switchyard.remote] successfully";
    
    private static final String REMOTE_BINDING_MSG = "Added binding.remote to service " + BEAN_SERVICE;
    
    private static final String REMOTE_REFERENCE_MSG = "Added binding.remote to reference " + BEAN_SERVICE_REFERENCEABLE;
    ...
    public static JavaArchive getDeployment() {
        ...
        archive.addPackages(true, RemoteFacet.class.getPackage());
    ...
    private void testBindService() throws Exception {
        resetOutputStream();
        getShell().execute("project install-facet switchyard.remote");
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(REMOTE_SUCCESS_MSG));
        resetOutputStream();
        ...
        getShell().execute("remote-binding bind-service --serviceName " + BEAN_SERVICE);
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(REMOTE_BINDING_MSG));
    }
    ...
    private void testBindReference() throws Exception {
        ...
        getShell().execute("remote-binding bind-reference --referenceName " + BEAN_SERVICE_REFERENCEABLE);
        getShell().execute("switchyard show-config");
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(REMOTE_REFERENCE_MSG));
    }
