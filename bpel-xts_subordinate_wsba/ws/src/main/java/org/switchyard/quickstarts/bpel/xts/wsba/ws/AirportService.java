...
public class AirportService {
    ...
    public void order(@WebParam(name = "name") String name, @WebParam(name = "fltid") String fltid) {
        ...
        if (uba != null) {
            ...
            OrderParticipant op = new OrderParticipant(uba.toString(), name, fltid);

            try {
                // Enlist order participant to the transaction
                BusinessActivityManager.getBusinessActivityManager().enlistForBusinessAgreementWithCoordinatorCompletion(op,
                    "org.switchyard.quickstarts.bpel.xts.wsba.ws:AirportService:" + name + ":" + fltid);
            } catch (Exception e) {
