...
public class RemoteActivator extends BaseActivator {
    ...
    public RemoteActivator(Configuration environment) {
        ...
        try {
            ...
            _log.info("Unable to resolve cache-container " + cacheName 
                   +  ".  <binding.remote> will not be available for services and references.");
        }
    }
    ...
    public ServiceHandler activateBinding(QName name, BindingModel config) {
        if (_registry == null) {
            throw new SwitchYardException("Cannot enable remote binding for " + name
                    + ".  No distributed cache is avaialble.");
        }
        if (config.isServiceBinding()) {
            return new RemoteServiceEndpoint(
                    (RemoteBindingModel)config, super.getServiceDomain(), _endpointPublisher, _registry);
        } else {
            return new RemoteServiceHandler((RemoteBindingModel)config, _registry);
        }
    }
